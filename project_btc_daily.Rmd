---
title: "ISYE 6402 Final project BTC analysis(daily) ARIMA"
output:
...
c.col<-"#FF8C00"
data.daily <- read.csv("BTC.csv", head = TRUE)
...
From looking at the time series plots we can see an increasing, then decreasing and again increasing trend as well as some seasonal patterns. The acf plot confirms the trend as well as some seasonal patterns. The time series doesn't seem to be stationary.
...
Statistical significance analysis showed that AR coefficients 1,4,7,8 and 9 are statistically significant. Among MA coefficients 1,4, 7 and 8 are significant. It follows that we can't use a simpler model since AR and MA coefficients corresponding to the highest lags (9 and 8 respectively) are significant.
...
legend(x="topleft",legend=c("Log Price","Fitted values"), lty=1,col=c("black",c.col),cex=0.7)
```
From the fitted values plot we can see that the model failed to fit the data and there is a lot of variance that wasn't captured by the model.
...
However, the variance of the residuals doesn't seem to be constant. Since we can see clusters with higher variance. The acf plots show that most of the values lie inside the confidence bands. The same is for PACF plots: most of the values lie within the confidence bands. 
...
The null hypothesis for Ljung-Box test is that the residuals are uncorrelated. Since we can see that we got a p-value smaller than 0.05, it indicates that we can reject the null hypothesis and our residuals are correlated with 0.05 confidence.
...
#porder <- tail(orders1,6)$p[1]
#qorder <- tail(orders1,6)$q[1]
#dorder <- tail(orders1,6)$d[1]
#aic_value <- tail(orders,6)$AIC[1]
...
The null hypothesis for Ljung-Box test is that the residuals are uncorrelated. Since we can see that we got a pretty small p-value, it indicates that we reject the null hypothesis and our residuals are probably correlated.
...
ACF and PACF plots don't show correlations since most of the values are inside the confidence band. 
...
We can see that the model performs well predictions since all the actual values are within the confidence bands. The rolling predictions show the better PM than n-ahead predictions.
