---
title: "ISYE 6402 Final project DOGE analysis(daily) ARIMA"
output:
...
c.col<-"#9932CC"
data.daily <- read.csv("DOGE.csv", head = TRUE)
...
Statistical significance analysis showed that all AR coefficients except for 6 and 7 are statistically significant. Among MA coefficients all are significant. It follows that we can use a simpler model since AR coefficient corresponding to the highest lag (7) isn't significant.
...
The null hypothesis for Ljung-Box test is that the residuals are uncorrelated. Since we can see that we got a p-value greater than 0.05, it indicates that we can not reject the null hypothesis and our residuals are probably uncorrelated.
...
The null hypothesis for Ljung-Box test is that the residuals are uncorrelated. Since we can see that we got a pretty small p-value, it indicates that we are rejecting the null hypothesis and our residuals are correlated.
