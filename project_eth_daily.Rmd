---
title: "ISYE 6402 Final project ETH analysis(daily) ARIMA"
output:
...
c.col<-"#20B2AA"
data.daily <- read.csv("ETH.csv", head = TRUE)
...
The null hypothesis for Ljung-Box test is that the residuals are uncorrelated. Since we can see that we got a pretty high p-value (greater than 0.05), it indicates that we can not reject the null hypothesis and our residuals are probably uncorrelated.
...
```
We can see that the model performs well predicting. All the actual values are within the 95% confidence interval.
...
#porder <- tail(orders1,2)$p[1]
#qorder <- tail(orders1,2)$q[1]
#dorder <- tail(orders1,2)$d[1]
#aic_value <- tail(orders,2)$AIC[1]
