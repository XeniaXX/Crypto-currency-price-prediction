---
title: "ISYE 6402 Final project BNB analysis(daily) ARIMA"
output:
...
c.col<-"#008000"
data.daily <- read.csv("BNB.csv", head = TRUE)
...
From looking at the time series plots we can see an increasing and then decreasing trend as well as some seasonal patterns. The acf plot confirms the trend as well as some seasonal patterns. The time series isn't stationary.
...
Statistical significance analysis showed that all AR coefficients are statistically significant. Among MA coefficients all are significant as well. It follows that we can't use a simpler model since AR and MA coefficients corresponding to the highest lags (6 and 4 respectively) are significant.
...
Augmented Dickey-Fuller Test shows that the residual processes is stationary (which is probably not true).
...
ymax = max(ubound)
par(mfrow=c(1,1))
plot(ts(daily.dif[nfit+1:n][1:(n-nfit)],start=1),type="l", ylim=c(ymin,ymax), xlab="Time", ylab="Price")
...
#porder <- tail(orders1,1)$p
#qorder <- tail(orders1,1)$q
#dorder <- tail(orders1,1)$d
#aic_value <- tail(orders,1)$AIC
...
The null hypothesis for Ljung-Box test is that the residuals are uncorrelated. Since we can see that we got a pretty high p-value, it indicates that we don't reject the null hypothesis and our residuals are probably uncorrelated.
